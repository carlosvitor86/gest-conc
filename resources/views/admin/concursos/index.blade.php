                ...
                <table class="table table-vcenter card-table">
                    ...
                    <tbody>
                        @forelse ($concursos as $concurso)
                            <tr>
                                <td><a href="{{ route('concursos.show', $concurso) }}">{{ $concurso->nome }}</a></td>
                                ...
                                <td class="text-muted">
                                    {{ \Carbon\Carbon::parse($concurso->data_homologacao)->format('d/m/Y') }}</td>
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        <a href="{{ route('concursos.show', $concurso) }}" class="btn btn-icon btn-primary"
                                        ...
                                        <a href="{{ route('concursos.edit', $concurso) }}" class="btn btn-icon btn-warning"
                                        ...
                                        <a href="#" class="btn btn-icon btn-danger" title="Excluir"
                                            data-bs-toggle="tooltip"
                                            onclick="event.preventDefault(); if(confirm('Tem certeza que deseja excluir este concurso?')) { document.getElementById('delete-form-{{ $concurso->id }}').submit(); }">
                                        ...
                                        <form id="delete-form-{{ $concurso->id }}"
                                            action="{{ route('concursos.destroy', $concurso) }}" method="POST"
                                            style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
